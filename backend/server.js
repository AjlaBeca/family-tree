...
const sanitizePerson = (person) => {
  ...
  cleaned.faculty = String(cleaned.faculty || "").trim();
  cleaned.burialPlace = String(cleaned.burialPlace || "").trim();
  cleaned.photo = String(cleaned.photo || "").trim();
  if (
    cleaned.photo.toLowerCase() === "[object object]" ||
    cleaned.photo.toLowerCase() === "%5bobject%20object%5d" ||
    cleaned.photo.toLowerCase() === "[object%20object]"
  ) {
    cleaned.photo = "";
  }
...
const normalizePersonInput = (body = {}, idOverride = 0) => {
  const toText = (value, fallback = "") => String(value ?? fallback);
  const normalizePhotoValue = (value) => {
    if (typeof value === "string") {
      const trimmed = value.trim();
      if (trimmed.toLowerCase() === "[object object]") return "";
      return trimmed;
    }
    if (value && typeof value === "object") {
      const src = String(value.src || "").trim();
      if (!src || src.toLowerCase() === "[object object]") return "";
      return src;
    }
    return "";
  };
  const genderRaw = toText(body.gender, "M").toUpperCase();
  return {
    id: parseId(idOverride || body.id),
    familyId: parseId(body.familyId),
    name: toText(body.name).trim(),
    gender: genderRaw === "F" ? "F" : "M",
    birthYear: toText(body.birthYear).trim(),
    ...
    faculty: toText(body.faculty).trim(),
    burialPlace: toText(body.burialPlace).trim(),
    photo: normalizePhotoValue(body.photo),
    bio: toText(body.bio),
    parent: parseId(body.parent),
    parent2: parseId(body.parent2),
    spouse: parseId(body.spouse),
    divorced: Number(body.divorced) ? 1 : 0,
    isPinned: Number(body.isPinned) ? 1 : 0,
    pinColor: toText(body.pinColor, "#f59e0b").trim() || "#f59e0b",
  };
};
...
app.get("/api/people", async (req, res) => {
  try {
    ...
    const params = [];
    let sql =
      "SELECT id, family_id as familyId, name, gender, birth_year as birthYear, death_year as deathYear, birth_place as birthPlace, occupation, studies, faculty, burial_place as burialPlace, photo, bio, parent, parent2, spouse, divorced, is_pinned as isPinned, pin_color as pinColor FROM people";
...
app.post("/api/people", async (req, res) => {
  try {
    ...
    const result = await run(
      `INSERT INTO people
        (family_id, name, gender, birth_year, death_year, birth_place, occupation, studies, faculty, burial_place, photo, bio, parent, parent2, spouse, divorced, is_pinned, pin_color)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
      , [
        familyId,
        name,
    ...
    const person = await get(
      `SELECT id, family_id as familyId, name, gender, birth_year as birthYear, death_year as deathYear,
        birth_place as birthPlace, occupation, studies, faculty, burial_place as burialPlace, photo, bio, parent, parent2, spouse, divorced, is_pinned as isPinned, pin_color as pinColor
...
app.put("/api/people/:id", async (req, res) => {
  try {
    ...
    const {
      ...
      isPinned,
      pinColor,
    } = normalizePersonInput(req.body, personId);
    ...
    await run(
      `UPDATE people
        SET family_id = ?, name = ?, gender = ?, birth_year = ?, death_year = ?, birth_place = ?, occupation = ?, studies = ?, faculty = ?, burial_place = ?, photo = ?, bio = ?, parent = ?, parent2 = ?, spouse = ?, divorced = ?, is_pinned = ?, pin_color = ?
        WHERE id = ?`,
      [
        familyId,
        name,
